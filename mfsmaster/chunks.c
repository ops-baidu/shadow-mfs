...
static uint32_t log_delete_file=0;
static uint32_t log_add_file=0;

#ifndef METARESTORE

static uint32_t log_multi_modify=0;
static uint32_t log_multi_truncate=0;
static uint32_t log_repair=0;
static uint32_t log_server_has_chunk=0;
static uint32_t	log_damaged=0;
static uint32_t log_got_delete_status=0;
static uint32_t log_got_replicate_status=0;
static uint32_t log_valid=0;
...
static void chunk_hlist_resize(serventry *eptr) {
    uint32_t i;
    uint32_t newsize = eptr->chunkhlist.size << 1;
    uint32_t newmask = newsize - 1;
    uint32_t rehashnum = 0;
    chunk_helem_t *newelemarray; 
    chunk_helem_t *elem, *newelem, *nextelem;

    MFSLOG(LOG_NOTICE,  "resize begin size:%lu num:%lu\n", 
        eptr->chunkhlist.size, eptr->chunkhlist.num);

    newelemarray = calloc(newsize, sizeof(chunk_helem_t));
    if(newelemarray == NULL) {
        MFSLOG(LOG_NOTICE, "alloc mem for new chunk hlist failed size:%lu\n", newsize);
        return;
    }

    for(i = 0; i < eptr->chunkhlist.size; i++) {
        uint32_t newkey;
        /**
          * deal the first elem in hlist which is actually mem of hlist table, we should not 
          * free the elem, instead we should free it by free all the table 
          */
        elem = &eptr->chunkhlist.elem[i];

        if(elem->chunk) {
			newkey = ((chunk *)elem->chunk)->chunkid & newmask;
            if(newelemarray[newkey].chunk) {
                newelem = malloc(sizeof(chunk_helem_t));
                newelem->next = newelemarray[newkey].next;
				newelem->chunk = elem->chunk;
                newelemarray[newkey].next = newelem;                   
            } else {
                newelemarray[newkey].chunk = elem->chunk;
            }
            elem->chunk = NULL;
            rehashnum++;
        } else {
            /* no elem in the hlist */
            assert(elem->next == NULL);
            continue;
        }

        /* rehash the elem which is alloced one by one */
        elem = elem->next;
        while(elem) {
            /* del from the old array */
            nextelem = elem->next;
            eptr->chunkhlist.elem[i].next = nextelem;            
            newkey = ((chunk *)elem->chunk)->chunkid & newmask;
            rehashnum++;

            /* just move the elem to new elem array */
            if(newelemarray[newkey].chunk) {
                elem->next = newelemarray[newkey].next;
                newelemarray[newkey].next = elem;                
            } else {
                newelemarray[newkey].chunk = elem->chunk;
                free(elem);
                elem = NULL;
    ...
    assert(rehashnum == eptr->chunkhlist.num);

    eptr->chunkhlist.size   = newsize;
    eptr->chunkhlist.mask = newsize - 1;
    free(eptr->chunkhlist.elem);
    eptr->chunkhlist.elem = newelemarray;

    MFSLOG(LOG_NOTICE,  "resize end size:%lu num:%lu\n", 
        eptr->chunkhlist.size, eptr->chunkhlist.num);

}

static inline int chunk_hlist_need_resize(serventry *eptr)
{
    return eptr->chunkhlist.num > eptr->chunkhlist.size;
}

static void chunk_hlist_add(serventry *eptr,  chunk *addchunk)
{
    uint64_t chunkid = addchunk->chunkid;
    uint32_t key = chunkid & eptr->chunkhlist.mask;
    chunk_helem_t * elem =  &eptr->chunkhlist.elem[key];

    while(elem) {
    	if(elem->chunk && ((chunk *)elem->chunk)->chunkid == chunkid) {
    		MFSLOG(LOG_NOTICE, "chunkid:%llu exist skip it\n", chunkid);
    		return;
    	}
    	elem = elem->next;
    }
    ...
    if(eptr->chunkhlist.elem[key].chunk) {
        elem = malloc(sizeof(elem));
        elem->chunk                   = addchunk;  
        elem->next                    = eptr->chunkhlist.elem[key].next;
        eptr->chunkhlist.elem[key].next = elem;
    } else {
        eptr->chunkhlist.elem[key].chunk = addchunk;
    }

    if(chunk_hlist_need_resize(eptr)) {
    	MFSLOG(LOG_NOTICE, "needresize size:%llu num:%llu\n", 
    		eptr->chunkhlist.size, eptr->chunkhlist.num);
    	chunk_hlist_resize(eptr);
    }
}

void chunk_hlist_free(serventry *eptr)
{
/*
    uint32_t i;
    chunk_helem_t *elem, *nextelem;

    for(i = 0; i < eptr->chunkhlist.size; i++) {
        elem = eptr->chunkhlist.elem[i].next;
        while(elem) {
            nextelem= elem->next;
            free(elem);
            elem = nextelem;
...
*/
    ...
    assert(eptr->chunkhlist.num == 0);

    free(eptr->chunkhlist.elem);
    eptr->chunkhlist.elem = NULL;
}

void chunk_hlist_del(serventry *eptr, uint64_t chunkid)
{
    uint32_t key = chunkid & eptr->chunkhlist.mask;
    chunk_helem_t *elem, *nextelem;
    ...
    elem = &eptr->chunkhlist.elem[key];
    if(elem->chunk && ((chunk *)elem->chunk)->chunkid == chunkid) {
        /* copy the chunkid and del the chunk from hlist */
        if(elem->next) {
            elem->chunk = elem->next->chunk;
            nextelem = elem->next;
    ...
    nextelem = elem->next;
    while(nextelem) {
		if(nextelem->chunk && ((chunk *)nextelem->chunk)->chunkid == chunkid) {
            elem->next = nextelem->next;
            free(nextelem);
            nextelem = NULL;
            eptr->chunkhlist.num--;
...
*/
	...
	if(allchunkcounts[c->goal][0] == 0) {
		//MFSLOG(LOG_NOTICE, "the allcount goal:%u copys:%u is  zero skip it\n", c->goal, 0);
	} else {
		allchunkcounts[c->goal][0]--;		
	}

	if(regularchunkcounts[c->goal][0] == 0) {
		//MFSLOG(LOG_NOTICE, "the regucount goal:%u copys:%u is  zero skip it\n", c->goal, 0);
	} else {
...
static inline void chunk_state_change(uint8_t oldgoal,uint8_t newgoal,uint8_t oldavc,uint8_t newavc,uint8_t oldrvc,uint8_t newrvc) {
	...
	if(allchunkcounts[oldgoal][oldavc] == 0) {
		//MFSLOG(LOG_NOTICE, "the allcount goal:%u copys:%u is  zero skip it\n", oldgoal, oldavc);
	} else {
		allchunkcounts[oldgoal][oldavc]--;		
	}

	if(regularchunkcounts[oldgoal][oldrvc] == 0) {
		//MFSLOG(LOG_NOTICE, "the regucount goal:%u copys:%u is  zero skip it\n", oldgoal, oldrvc);
	} else {
	...
	*regularvalidcopies = 0;
	for (i=1 ; i<=10 ; i++) {
		...
		for (j=0 ; j<=10 ; j++) {
            ...
            //MFSLOG(LOG_NOTICE, "allchunk[%u][%u]:%u\n", j, i, allchunkcounts[j][i]);
	...
	for (s=c->slisthead ;s ; s=s->next) {
		if (s->valid!=INVALID && s->valid!=DEL) {
			if (cnt<100 && matocsserv_getlocation(s->ptr,&(lstab[cnt].ip),&(lstab[cnt].port))==0) {
				exact_dist = lstab[cnt].ip^cuip;
				std_dist=0;
				while(exact_dist){
					exact_dist = exact_dist>>8;
					std_dist++;
				}
				lstab[cnt].dist = std_dist;    // in the future prepare more sofisticated distance function
...
void chunk_server_has_chunk(void *ptr,uint64_t chunkid,uint32_t version) {
	...
	c = chunk_find(chunkid);
	if (c==NULL) {
		log_server_has_chunk %= HAS_CHUNK_COUNT;
...
*/
        ...
        MFSLOG(LOG_WARNING,"begin");
        for (i=0 ; i<eptr->chunkhlist.size; i++) {
        chunk_helem_t *elem     = &eptr->chunkhlist.elem[i];
        chunk_helem_t *nextelem = NULL;
        ...
        while(elem) {
            /* del and free it from the hlist which is not the first elem of the 
              * hlist, later all the hlist will be freed before tcp_close the eptr
              */
            c = elem->chunk; 
            nextelem = elem->next;
            ...
            while (*st) {
                ...
                if (s->ptr == ptr) {
                    if (s->valid==TDBUSY || s->valid==TDVALID) {
                        chunk_state_change(c->goal,c->goal,c->allvalidcopies,
                            c->allvalidcopies-1,c->regularvalidcopies,c->regularvalidcopies);
                        c->allvalidcopies--;
                    }
                    if (s->valid==BUSY || s->valid==VALID) {
            ...
            if(elem != &eptr->chunkhlist.elem[i]) {
                eptr->chunkhlist.elem[i].next = nextelem;
				//MFSLOG(LOG_WARNING,"del second chunid:%llu", ((chunk *)elem->chunk)->chunkid);
                free(elem);
            } else {
				//MFSLOG(LOG_WARNING,"del first chunid:%llu", ((chunk *)elem->chunk)->chunkid);
                elem->chunk = NULL;
            }
...
int chunk_is_good_deletion(chunk *c, void *del) {
    slist* slist_iter;
    serventry *eptr_iter, *eptr_cmp, *eptr_del;
    eptr_cmp = NULL;
    eptr_del = (serventry *)del;

    for(slist_iter=c->slisthead; slist_iter; slist_iter=slist_iter->next) {
        if(slist_iter->ptr == del) {
            continue;
        }
        if(slist_iter->valid != VALID && slist_iter->valid != BUSY) {
            continue;
        }
        eptr_iter = (serventry *)slist_iter->ptr;
        if(eptr_cmp == NULL) {
            eptr_cmp = eptr_iter;
        }
        else if(!net_is_same_rack(eptr_iter->servip, eptr_cmp->servip)) {
            return 1;
        }
        if(net_is_same_rack(eptr_iter->servip, eptr_del->servip)) {
            return 1;
        }
    }

    if(c->regularvalidcopies <= 2) {
        return 1;
    }
    return 0;
}

int chunk_is_multirack(chunk *c) {
    slist *slist_iter;
    serventry *eptr_iter, *eptr_head;
    eptr_head = (serventry *)c->slisthead->ptr;
    ...
    for(slist_iter=c->slisthead; slist_iter; slist_iter=slist_iter->next) {
        ...
        eptr_iter = (serventry *)slist_iter->ptr;
        if(!net_is_same_rack(eptr_iter->servip, eptr_head->servip)) {
            return 1;
        }
    }
    return 0;
...
int chunk_is_good_recovery(chunk *c, void* recovery) {
    slist *slist_iter;
    serventry *eptr_iter, *eptr_recovery;

    eptr_iter = NULL;
    eptr_recovery = (serventry *)recovery;
    for(slist_iter=c->slisthead; slist_iter; slist_iter=slist_iter->next) {
        if(slist_iter->valid != VALID && slist_iter->valid != BUSY) {
            continue;
        }
        eptr_iter = (serventry *)slist_iter->ptr;
        if(!net_is_same_rack(eptr_recovery->servip, eptr_iter->servip)) {
            return 1;
        }
    }
    if(c->regularvalidcopies == 0) {
...
int chunk_is_good_move(chunk* c, void* src, void* dst) {
    slist *slist_iter;
    serventry *eptr_src, *eptr_dst, *eptr_iter;
    eptr_src = (serventry *)src;
    eptr_dst = (serventry *)dst;

    if(net_is_same_rack(eptr_src->servip, eptr_dst->servip)) {
        return 1;
    }
    for(slist_iter=c->slisthead; slist_iter; slist_iter=slist_iter->next) {
        if(slist_iter->ptr == src) {
            continue;
        }
        if(slist_iter->valid != VALID && slist_iter->valid != BUSY) {
            continue;
        ...
        eptr_iter = (serventry *)slist_iter->ptr;
        if(net_is_same_rack(eptr_iter->servip, eptr_src->servip)) {
            return 1;
        }
        if(!net_is_same_rack(eptr_iter->servip, eptr_dst->servip)) {
            return 1;
        }
    }
    if(c->regularvalidcopies <= 1) {
        return 1;
    }
    return 0;
}
...
extern serventry *matocsservhead;
...
// step 3. delete invalid copies
	if (delcount<TmpMaxDel) {
		for (s=c->slisthead ; s ; s1=s,s=s->next) {
			if (s->valid==INVALID || s->valid==DEL) {
				if (s->valid==DEL) {
					MFSLOG(LOG_WARNING,"chunk hasn't been deleted since previous loop - retry");
                           ...
                           /**
                             *  somewhere the eptr has been released, but the slist is not released 
                             * 
                             *  FixMe!
                             *  Dongyang Zhang
                             */
                    	for (eptr = matocsservhead ; eptr; eptr=eptr->next) {
                           ...
                           if(!eptr) {
                               MFSLOG(LOG_WARNING, "the eptr has been delete but the slist is not released\n");
                               if(s == c->slisthead) {
                                   c->slisthead = s->next;      
                               } else {
                                   s1->next = s->next;
...
//		syslog(LOG_WARNING,"vc (%"PRIu32") > goal (%"PRIu32") - delete",vc,c->goal);
		if (delcount<TmpMaxDel) {
			...
			for (i=0 ; i<servcount && vc>c->goal ; i++) {
				...
				if (s && s->valid==VALID && chunk_is_good_deletion(c, s->ptr)) {
					chunk_state_change(c->goal,c->goal,c->allvalidcopies,c->allvalidcopies-1,c->regularvalidcopies,c->regularvalidcopies-1);
					c->allvalidcopies--;
					c->regularvalidcopies--;
					c->needverincrease=1;
...
//step 8. if chunk has number of copies less than goal then make another copy of this chunk
	if (c->goal > vc && vc+tdc > 0) {
		if (jobsnorepbefore<(uint32_t)get_current_time()) {
			uint32_t rgvc,rgtdc;
            int server_multirack,availserv_multirack;
			rservcount = matocsserv_getservers_lessrepl(rptrs, &server_multirack, &availserv_multirack, MaxWriteRepl);
			rgvc=0;
			rgtdc=0;
			for (s=c->slisthead ; s ; s=s->next) {
			...
			if (rgvc+rgtdc>0 && rservcount>0) { // have at least one server to read from and at least one to write to
				for (i=0 ; i<rservcount ; i++) {
                    ...
                    if(!chunk_is_good_recovery(c, rptrs[i])) {
                        if(availserv_multirack) {
                            continue;
                        } else if (server_multirack) {
                            break;
                        }
                    }
					uint32_t r;
				...
				if (min>0) {
					for (i=0 ; i<min && dstserv==NULL ; i++) {
						if (matocsserv_replication_write_counter(ptrs[i])<MaxWriteRepl) {
							for (s=c->slisthead ; s && s->ptr!=ptrs[i] ; s=s->next ) {}
							if (s==NULL && chunk_is_good_move(c, srcserv, ptrs[i])) {
								dstserv=ptrs[i];
							}
						}
					}
...
void log_print_control_ck(void) {
	...
	log_add_file=0;
	log_multi_modify=0;
	log_multi_truncate=0;
	log_repair=0;
	log_server_has_chunk=0;
	log_damaged=0;
	log_got_delete_status=0;
	log_got_replicate_status=0;
	log_valid=0;
}
...
void chunk_jobs_main(void) {
	...
	if (tscount<lasttscount) {		// servers disconnected
		jobsnorepbefore = get_current_time()+ReplicationsDelayDisconnect;
	} else if (tscount>lasttscount) {	// servers connected
		if (tscount>=maxtscount) {
			maxtscount = tscount;
			jobsnorepbefore = get_current_time();
		}
	} else if (tscount<maxtscount && (uint32_t)get_current_time()>jobsnorepbefore) {
		maxtscount = tscount;
	}
	lasttscount = tscount;
...
int slave_chunk_delete_file(uint64_t chunkid,uint32_t inode,uint16_t indx) {
        chunk *c;
        flist *f,**fp;
        uint32_t i;     
        c = chunk_find(chunkid);
...
//slavemaster chunk op interface
int slave_chunk_add_file(uint64_t chunkid,uint32_t inode,uint16_t indx,uint8_t goal) {
        chunk *c;               
        flist *f;               
	uint32_t i;
        c = chunk_find(chunkid);
...
int slave_chunk_multi_modify(uint32_t ts,uint64_t *nchunkid,uint64_t ochunkid,uint32_t inode,uint16_t indx,uint8_t goal,uint8_t opflag) {
	uint32_t i;
	chunk *oc,*c;
	flist *f,**fp;
...
//		servcount = matocsserv_getservers_ordered(ptrs,MINMAXRND,NULL,NULL);
		...
		*nchunkid = c->chunkid;
		...
		/**
		 * recompute counts,  otherwise in the chunk_server_has_chunk we will 
		 * decrese the allchunkcounts[c->goal][i] and increase [c->goal][i+1] 
		 * which would lead the count to negative 
		 *
		 * Dongyang Zhang
		 */
#ifndef METARESTORE
		...
		if (i && f && f->next==NULL) {	// refcount==1
			*nchunkid = ochunkid;
			...
			if (f==NULL) {	// it's serious structure error
				MFSLOG(LOG_WARNING,"serious structure inconsistency: (chunkid:%016"PRIX64" "
                        "; inode:%"PRIu32" ; index:%"PRIu16")",ochunkid,inode,indx);
                return ERROR_CHUNKLOST;	// ERROR_STRUCTURE
			}
			c = chunk_new(nextchunkid++);
			c->version = 1;
	...
	c->lockedto=ts+LOCKTIMEOUT;
	//c->lockedto=get_current_time()+LOCKTIMEOUT;
	return STATUS_OK;
}

int slave_chunk_multi_truncate(uint32_t ts,uint64_t *nchunkid,uint64_t ochunkid,uint32_t inode,uint16_t indx,uint8_t goal) {
    chunk *oc,*c;
    flist *f,**fp; 
    uint32_t i;
    ...
    if (i && f && f->next==NULL) {  // refcount==1
        *nchunkid = ochunkid;
        ...
        if (f==NULL) {  // it's serious structure error
            MFSLOG(LOG_WARNING, "serious structure inconsistency: (chunkid:%016"PRIX64" ; inode:%"PRIu32" "
                    " ; index:%"PRIu16")\n", ochunkid, inode, indx);
            return ERROR_CHUNKLOST;	// ERROR_STRUCTURE
        }
        c = chunk_new(nextchunkid++);
...
int slave_chunk_set_version(uint64_t chunkid,uint32_t version) {
	chunk *c;
	c = chunk_find(chunkid);
	if (c==NULL) {
		return ERROR_NOCHUNK;
...
#ifdef UNITTEST
void* chunk_unittest_delete_overgoal(void *chunk_ptr) {
    chunk *c = (chunk *)chunk_ptr;
    uint16_t servcount;
	slist *s;
	void* ptrs[65535];
	uint16_t i;
...
void* chunk_unittest_recovery_select(void *chunk_ptr) {
    chunk *c = (chunk *)chunk_ptr;
	slist *s;
	void *srcptr;
	void* rptrs[65536];
...
int chunk_unittest_balance(void *chunk_ptr, void **pptr_src, void **pptr_dst) {
	slist *s;
    chunk *c = (chunk *)chunk_ptr;
	void* ptrs[65535];
	static uint32_t min,max;
...
void* chunk_unittest_create_by_servlist(void *ptrs[], uint8_t repnum, uint8_t goal) {
    uint16_t i;
    slist* s;
    chunk* c = chunk_new(nextchunkid++);
    c->goal = goal;
    c->version = 1;
	c->interrupted = 0;
	c->allvalidcopies = repnum;
	c->regularvalidcopies = repnum;
	for (i=0 ; i<c->allvalidcopies ; i++) {
		s = slist_malloc();
		s->ptr = ptrs[i];
		s->valid = VALID;
...
void chunk_uinttest_init(uint32_t readrep, uint32_t writerep) {
    MaxReadRepl = readrep;
    MaxWriteRepl = writerep;
}
#endif
