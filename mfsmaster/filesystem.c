...
uint32_t log_structure_error=0;
uint32_t log_test_files=0;
uint32_t LOG_COUNT;
...
uint32_t fsinfo_files=0;
uint32_t fsinfo_ugfiles=0;
uint32_t fsinfo_mfiles=0;
uint32_t fsinfo_chunks=0;
uint32_t fsinfo_ugchunks=0;
uint32_t fsinfo_mchunks=0;
char *fsinfo_msgbuff=NULL;
uint32_t fsinfo_msgbuffleng=0;
uint32_t fsinfo_loopstart=0;
uint32_t fsinfo_loopend=0;

uint32_t starttime;

uint32_t stats_statfs=0;
uint32_t stats_getattr=0;
uint32_t stats_setattr=0;
uint32_t stats_lookup=0;
uint32_t stats_mkdir=0;
uint32_t stats_rmdir=0;
uint32_t stats_symlink=0;
uint32_t stats_readlink=0;
uint32_t stats_mknod=0;
uint32_t stats_unlink=0;
uint32_t stats_rename=0;
uint32_t stats_link=0;
uint32_t stats_readdir=0;
uint32_t stats_open=0;
uint32_t stats_read=0;
uint32_t stats_write=0;
...
freenode_bucket *fnbhead = NULL;
freenode *fnfreehead = NULL;
...
sessionidrec_bucket *crbhead = NULL;
sessionidrec *crfreehead = NULL;
...
#ifndef METARESTORE
	changelog(version++,"%"PRIu32"|FREEINODES():%"PRIu32,(uint32_t)get_current_time(),fi);
#else
	version++;
	if (freeinodes!=fi) {
		return 1;
...
#ifndef  METARESTORE
	/* the slave side's connection forbid reference count */
	if(isslave()) {
		return STATUS_OK;
	}
#endif
...
#ifndef METARESTORE
	MFSLOG(LOG_WARNING,"release: session not found");
#endif
	return ERROR_EINVAL;
}
...
uint8_t fs_reinitchunk(uint32_t inode,uint32_t indx,uint64_t *chunkid) {
	...
	*chunkid = 0;
	...
	changelog(version++,"%"PRIu32"|REINIT(%"PRIu32",%"PRIu32"):%"PRIu64,(uint32_t)get_current_time(),inode,indx,nchunkid);
	*chunkid = nchunkid;
	p->mtime = p->ctime = get_current_time();
	return STATUS_OK;
}
...
uint8_t fs_eattr(uint32_t rootinode,uint8_t sesflags,uint32_t inode,uint32_t uid,uint8_t delflag,uint8_t *nodeeattr,uint8_t *functioneattr) {
	...
	if (*nodeeattr) {
		...
		changelog(version++,"%"PRIu32"|EATTR(%"PRIu32",%"PRIu16")",get_current_time(),inode,p->mode>>12);
		p->ctime = get_current_time();
	}
	*nodeeattr = p->mode>>12;
...
uint32_t fs_test_log_inconsistency(fsedge *e,const char *iname,char *buff,uint32_t size) {
	...
	if (e->parent) {
	        log_structure_error %= LOG_COUNT;
                if (log_structure_error++ == 0){	
			MFSLOG(LOG_ERR,"structure error - %s inconsistency (edge: %"PRIu32",%s -> %"PRIu32")",iname,e->parent->id,fsnodes_escape_name(e->nleng,e->name),e->child->id);
		}
		if (leng<size) {
		...
		if (e->child->type==TYPE_TRASH) {
		        log_structure_error %= LOG_COUNT;
                        if (log_structure_error++ == 0){
				MFSLOG(LOG_ERR,"structure error - %s inconsistency (edge: TRASH,%s -> %"PRIu32")",iname,fsnodes_escape_name(e->nleng,e->name),e->child->id);
			}
			if (leng<size) {
			...
			log_structure_error %= LOG_COUNT;
                        if (log_structure_error++ == 0){
				MFSLOG(LOG_ERR,"structure error - %s inconsistency (edge: RESERVED,%s -> %"PRIu32")",iname,fsnodes_escape_name(e->nleng,e->name),e->child->id);
			}
			if (leng<size) {
				leng += snprintf(buff+leng,size-leng,"structure error - %s inconsistency (edge: RESERVED,%s -> %"PRIu32")\n",iname,fsnodes_escape_name(e->nleng,e->name),e->child->id);
                       	...
                       	log_structure_error %= LOG_COUNT;
                       	if (log_structure_error++ == 0){
				MFSLOG(LOG_ERR,"structure error - %s inconsistency (edge: NULL,%s -> %"PRIu32")",iname,fsnodes_escape_name(e->nleng,e->name),e->child->id);
			}
			if (leng<size) {
	...
	if (i==0) {
		if (errors==ERRORS_LOG_MAX) {
			log_test_files %= LOG_COUNT;
                        if (log_test_files++ == 0){
				MFSLOG(LOG_ERR,"only first %u errors (unavailable chunks/files) were logged",ERRORS_LOG_MAX);
			}	
			if (leng<MSGBUFFSIZE) {
		...
		if (notfoundchunks>0) {
			log_test_files %= LOG_COUNT;
                        if (log_test_files++ == 0){
				MFSLOG(LOG_ERR,"unknown chunks: %"PRIu32,notfoundchunks);
			}
			if (leng<MSGBUFFSIZE) {
		...
		if (unavailchunks>0) {
			log_test_files %= LOG_COUNT;
                        if (log_test_files++ == 0){
				MFSLOG(LOG_ERR,"unavailable chunks: %"PRIu32,unavailchunks);
			}
			if (leng<MSGBUFFSIZE) {
		...
		if (unavailtrashfiles>0) {
			log_test_files %= LOG_COUNT;
                        if (log_test_files++ == 0){
				MFSLOG(LOG_ERR,"unavailable trash files: %"PRIu32,unavailtrashfiles);
			}	
			if (leng<MSGBUFFSIZE) {
		...
		if (unavailreservedfiles>0) {
			log_test_files %= LOG_COUNT;
                        if (log_test_files++ == 0){
				MFSLOG(LOG_ERR,"unavailable reserved files: %"PRIu32,unavailreservedfiles);
			}
			if (leng<MSGBUFFSIZE) {
		...
		if (unavailfiles>0) {
			log_test_files %= LOG_COUNT;
                        if (log_test_files++ == 0){
				MFSLOG(LOG_ERR,"unavailable files: %"PRIu32,unavailfiles);
			}
			if (leng<MSGBUFFSIZE) {
					...
					if (chunkid>0) {
						if (chunk_get_validcopies(chunkid,&vc)!=STATUS_OK) {
							if (errors<ERRORS_LOG_MAX) {
					                        log_test_files %= LOG_COUNT;
                        					if (log_test_files++ == 0){
									MFSLOG(LOG_ERR,"structure error - chunk %016"PRIX64" not found (inode: %"PRIu32" ; index: %"PRIu32")",chunkid,f->id,j);
								}
								if (leng<MSGBUFFSIZE) {
							...
							if (errors<ERRORS_LOG_MAX) {
					                        log_test_files %= LOG_COUNT;
                        					if (log_test_files++ == 0){
									MFSLOG(LOG_ERR,"currently unavailable chunk %016"PRIX64" (inode: %"PRIu32" ; index: %"PRIu32")",chunkid,f->id,j);
								}
								if (leng<MSGBUFFSIZE) {
				...
				if (valid==0) {
					...
					if (f->type==TYPE_TRASH) {
						if (errors<ERRORS_LOG_MAX) {
				                        log_test_files %= LOG_COUNT;
                        				if (log_test_files++ == 0){
								MFSLOG(LOG_ERR,"- currently unavailable file in trash %"PRIu32": %s",f->id,fsnodes_escape_name(f->parents->nleng,f->parents->name));
							}
							if (leng<MSGBUFFSIZE) {
						...
						if (errors<ERRORS_LOG_MAX) {
				                        log_test_files %= LOG_COUNT;
                        				if (log_test_files++ == 0){
								MFSLOG(LOG_ERR,"+ currently unavailable reserved file %"PRIu32": %s",f->id,fsnodes_escape_name(f->parents->nleng,f->parents->name));
							}
							if (leng<MSGBUFFSIZE) {
						...
						for (e=f->parents ; e ; e=e->nextparent) {
							if (errors<ERRORS_LOG_MAX) {
								fsnodes_getpath(e,&pleng,&path);
								MFSLOG(LOG_ERR,"* currently unavailable file %"PRIu32": %s",f->id,fsnodes_escape_name(pleng,path));
								if (leng<MSGBUFFSIZE) {
			...
			for (e=f->parents ; e ; e=e->nextparent) {
				if (e->child != f) {
					if (e->parent) {
						log_test_files %= LOG_COUNT;
                        			if (log_test_files++ == 0){
							MFSLOG(LOG_ERR,"structure error - edge->child/child->edges (node: %"PRIu32" ; edge: %"PRIu32",%s -> %"PRIu32")",f->id,e->parent->id,fsnodes_escape_name(e->nleng,e->name),e->child->id);
						}
						if (leng<MSGBUFFSIZE) {
							leng += snprintf(msgbuff+leng,MSGBUFFSIZE-leng,"structure error - edge->child/child->edges (node: %"PRIu32" ; edge: %"PRIu32",%s -> %"PRIu32")\n",f->id,e->parent->id,fsnodes_escape_name(e->nleng,e->name),e->child->id);
				                ...
				                log_test_files %= LOG_COUNT;
                        			if (log_test_files++ == 0){
							MFSLOG(LOG_ERR,"structure error - edge->child/child->edges (node: %"PRIu32" ; edge: NULL,%s -> %"PRIu32")",f->id,fsnodes_escape_name(e->nleng,e->name),e->child->id);
						}
						if (leng<MSGBUFFSIZE) {
				...
				for (e=f->data.ddata.children ; e ; e=e->nextchild) {
					if (e->parent != f) {
						if (e->parent) {
							log_test_files %= LOG_COUNT;
                        				if (log_test_files++ == 0){
								MFSLOG(LOG_ERR,"structure error - edge->parent/parent->edges (node: %"PRIu32" ; edge: %"PRIu32",%s -> %"PRIu32")",f->id,e->parent->id,fsnodes_escape_name(e->nleng,e->name),e->child->id);
							}
							if (leng<MSGBUFFSIZE) {
							...
							log_test_files %= LOG_COUNT;
                        				if (log_test_files++ == 0){
								MFSLOG(LOG_ERR,"structure error - edge->parent/parent->edges (node: %"PRIu32" ; edge: NULL,%s -> %"PRIu32")",f->id,fsnodes_escape_name(e->nleng,e->name),e->child->id);
							}
							if (leng<MSGBUFFSIZE) {
...
void fs_store(FILE *fd) {
    ...
    MFSLOG(LOG_NOTICE, "store version:%llu\n", version);
...
#endif
		if(i < 0) {
			MFSLOG(LOG_ERR, "for failed i:%d errno:%d\n", i, errno);
		}
...
void fs_term(void) {
	...
	for (u=0 ; u<3 ; u++) {
		if (fs_storeall(0)==1) {
			if (rename("metadata.mfs.back","metadata.mfs")<0) {
				MFSLOG(LOG_WARNING,"can't rename metadata.mfs.back -> metadata.mfs (%m)");
			}
			return ;
		}
		sleep(5);
	...
	while (fs_emergency_saves()<0) {
		MFSLOG(LOG_ERR,"can't store metadata - try to make more space on your hdd or change privieleges");
		sleep(10);
	}
}
...
#define PATH_MAX 10000
...
#endif
			if (cwdbuf[0]) {
				MFSLOG(LOG_NOTICE,"if this is new instalation then rename %smetadata.mfs.empty as %smetadata.mfs\n",cwdbuf,cwdbuf);
			} else {
	...
	if (fread(hdr,1,8,fd)!=8) {
			MFSLOG(LOG_NOTICE,"can't read metadata header\n");
#ifndef METARESTORE
...
*/
	...
	if (memcmp(hdr,"MFSM 1.5",8)==0) {
		...
		MFSLOG(LOG_NOTICE,"loading chunks data ... ");
		...
		MFSLOG(LOG_NOTICE,"wrong metadata header\n");
#ifndef METARESTORE
	...
	if (ferror(fd)!=0) {
		MFSLOG(LOG_NOTICE,"error reading metadata\n");
#ifndef METARESTORE
...
#endif
	MFSLOG(LOG_NOTICE,"connecting files and chunks ... ");
	fs_add_files_to_chunks();
	MFSLOG(LOG_NOTICE,"ok\n");
#ifndef METARESTORE
	MFSLOG(LOG_NOTICE,"all inodes: %"PRIu32"\n",nodes);
	MFSLOG(LOG_NOTICE,"directory inodes: %"PRIu32"\n",dirnodes);
	MFSLOG(LOG_NOTICE,"file inodes: %"PRIu32"\n",filenodes);
	MFSLOG(LOG_NOTICE,"chunks: %"PRIu32"\n",chunk_count());
#endif
	return 0;
}
...
int fs_init() {
	...
	/* slave will load metadata when finish got changelog from master */
	if(ismaster()) {
		fs_strinit();
		chunk_strinit();
...
//	config_getuint32("QUOTA_TIME_LIMIT",7*86400,&QuotaTimeLimit);
	...
	/* slave only need to store the image which actually need not */
	if(ismaster()) {
...
#ifndef METARESTORE
uint8_t slave_fs_freeinodes(uint32_t ts,uint32_t freeinodes) {
        uint32_t fi,now,pos,mask;
        freenode *n,*an;
	now = ts;
	fi = 0;
	...
	version++;
        if (freeinodes!=fi) {
//		syslog(LOG_NOTICE,"slave_fs_freeinodes failed:fi is %d,freeinodes is %d",fi,freeinodes);
                return 1;
        }
        return 0;
}

static inline void slave_fsnodes_remove_edge(uint32_t ts,fsedge *e) {
        if (e->parent) {
                e->parent->mtime = e->parent->ctime = ts;
                e->parent->data.ddata.elements--;
                if (e->child->type==TYPE_DIRECTORY) {
...
static inline uint8_t slave_fsnodes_appendchunks(uint32_t ts,fsnode *dstobj,fsnode *srcobj) {
	uint64_t chunkid,length;
	uint32_t i;
	uint32_t srcchunks,dstchunks;
	statsrecord psr,nsr;
	...
	for (i=0 ; i<srcchunks ; i++) {
		...
		dstobj->data.fdata.chunktab[i+dstchunks] = chunkid;
		if (chunkid>0) {
			if (slave_chunk_add_file(chunkid,dstobj->id,i+dstchunks,dstobj->goal)!=STATUS_OK) {
				log_structure_error %= LOG_COUNT;
				if (log_structure_error++ == 0){
					 MFSLOG(LOG_ERR,"structure error - chunk %016"PRIX64" not found (inode: %"PRIu32" ; index: %"PRIu32")",chunkid,srcobj->id,i);
				}
...
static inline void slave_fsnodes_setlength(fsnode *obj,uint64_t length) {
        uint32_t i,chunks;
        uint64_t chunkid;
        fsedge *e;
        statsrecord psr,nsr;
        ...
        for (i=chunks ; i<obj->data.fdata.chunks ; i++) { 
                chunkid = obj->data.fdata.chunktab[i];
                if (chunkid>0) {
                        if (slave_chunk_delete_file(chunkid,obj->id,i)!=STATUS_OK) {
                                log_structure_error %= LOG_COUNT;
                                if (log_structure_error++ == 0){
                                        MFSLOG(LOG_ERR,"structure error - chunk %016"PRIX64" not found (inode: %"PRIu32" ; index: %"PRIu32")",chunkid,obj->id,i);
                                }
...
static inline void slave_fsnodes_remove_node(uint32_t ts,fsnode *toremove) {
        uint32_t nodepos;
        fsnode **ptr;
        if (toremove->parents!=NULL) {
                return;
...
// and free     
        ...
        if (toremove->type==TYPE_FILE || toremove->type==TYPE_TRASH || toremove->type==TYPE_RESERVED) {
                ...
                for (i=0 ; i<toremove->data.fdata.chunks ; i++) {
                        chunkid = toremove->data.fdata.chunktab[i];
                        if (chunkid>0) {
                                if (slave_chunk_delete_file(chunkid,toremove->id,i)!=STATUS_OK) {
                                        log_structure_error %= LOG_COUNT;
                                        if (log_structure_error++ == 0){
                                                MFSLOG(LOG_ERR,"structure error - chunk %016"PRIX64" not found (inode: %"PRIu32" ; index: %"PRIu32")",chunkid,toremove->id,i);
                                        }
...
static inline void slave_fsnodes_unlink(uint32_t ts,fsedge *e) {
	fsnode *child;
	uint16_t pleng=0;
	uint8_t *path=NULL;
	...
	slave_fsnodes_remove_edge(ts,e);
	if (child->parents==NULL) {	// last link
		if (child->type == TYPE_FILE) {
			if (child->trashtime>0) {
				child->type = TYPE_TRASH;
				...
				slave_fsnodes_remove_node(ts,child);
			}
		} else {
			slave_fsnodes_remove_node(ts,child);
		}
...
static inline int slave_fsnodes_purge(uint32_t ts,fsnode *p) {
        fsedge *e;
        e = p->parents;
        ...
        if (p->type==TYPE_TRASH) {
                ...
                if (p->data.fdata.sessionids!=NULL) {
                        ...
                        *(e->prevchild) = e->nextchild;
                        ...
                        slave_fsnodes_remove_edge(ts,e);
                        slave_fsnodes_remove_node(ts,p);
                        return 1;
                }
        } else if (p->type==TYPE_RESERVED) {
                reservedspace -= p->data.fdata.length;
...
static inline void slave_fsnodes_snapshot(uint32_t ts,fsnode *srcnode,fsnode *parentnode,uint32_t nleng,const uint8_t *name) {
	fsedge *e;
	fsnode *dstnode;
	uint32_t i;
	uint64_t chunkid;
	if ((e=fsnodes_lookup(parentnode,nleng,name))) {
		...
		if (srcnode->type==TYPE_DIRECTORY) {
			for (e = srcnode->data.ddata.children ; e ; e=e->nextchild) {
				slave_fsnodes_snapshot(ts,e->child,dstnode,e->nleng,e->name);
			}
		} else if (srcnode->type==TYPE_FILE) {
			uint8_t same;
			if (dstnode->data.fdata.length==srcnode->data.fdata.length && dstnode->data.fdata.chunks==srcnode->data.fdata.chunks) {
...
//				dstnode->mtime = srcnode->mtime;
				...
				for (i=0 ; i<srcnode->data.fdata.chunks ; i++) {
					...
					dstnode->data.fdata.chunktab[i] = chunkid;
					if (chunkid>0) {
						if (slave_chunk_add_file(chunkid,dstnode->id,i,dstnode->goal)!=STATUS_OK) {
                                			log_structure_error %= LOG_COUNT;
                                			if (log_structure_error++ == 0){	
								MFSLOG(LOG_ERR,"structure error - chunk %016"PRIX64" not found (inode: %"PRIu32" ; index: %"PRIu32")",chunkid,srcnode->id,i);
							}
...
uint8_t slave_fs_access(uint32_t ts,uint32_t inode) {
	fsnode *p;
	p = fsnodes_id_to_node(inode);
	if (!p) {
		return ERROR_ENOENT;
...
uint8_t slave_fs_setpath(uint32_t inode,const uint8_t *path) {
	uint32_t pleng;
	fsnode *p;
	uint8_t *newpath;
	pleng = strlen((char*)path);
...
uint8_t slave_fs_undel(uint32_t ts,uint32_t inode) {
        fsnode *p;
        uint8_t status;
	p = fsnodes_id_to_node(inode);
        if (!p) {
...
uint8_t slave_fs_purge(uint32_t ts,uint32_t inode) {
	fsnode *p;
	p = fsnodes_id_to_node(inode);
        if (!p) {
                return ERROR_ENOENT;
        ...
        slave_fsnodes_purge(ts,p);
	version++;
	return STATUS_OK;
}

uint8_t slave_fs_trunc(uint32_t ts,uint32_t inode,uint32_t indx,uint64_t chunkid) {
	uint64_t ochunkid,nchunkid;
	uint8_t status;
	fsnode *p;
	p = fsnodes_id_to_node(inode);
	...
	ochunkid = p->data.fdata.chunktab[indx];
	status = slave_chunk_multi_truncate(ts,&nchunkid,ochunkid,inode,indx,p->goal);
	if (status!=STATUS_OK) {
		return status;
	}
	if (chunkid!=nchunkid) {
...
uint8_t slave_fs_unlock(uint64_t chunkid) {
	version++;
	return chunk_unlock(chunkid);
}

uint8_t slave_fs_attr(uint32_t ts,uint32_t inode,uint32_t mode,uint32_t uid,uint32_t gid,uint32_t atime,uint32_t mtime) {
	fsnode *p;
	p = fsnodes_id_to_node(inode);
	if (!p) {
		return ERROR_ENOENT;
...
uint8_t slave_fs_length(uint32_t ts,uint32_t inode,uint64_t length) {
	fsnode *p;
	p = fsnodes_id_to_node(inode);
	if (!p) {
		return ERROR_ENOENT;
	...
	slave_fsnodes_setlength(p,length);
	p->mtime = ts;
	p->ctime = ts;
	version++;
	return STATUS_OK;
...
uint8_t slave_fs_symlink(uint32_t ts,uint32_t parent,uint32_t nleng,const uint8_t *name,const uint8_t *path,uint32_t uid,uint32_t gid,uint32_t inode) {
	uint32_t pleng;
        fsnode *wd,*p;
        uint8_t *newpath;
...
uint8_t slave_fs_create(uint32_t ts,uint32_t parent,uint32_t nleng,const uint8_t *name,uint8_t type,uint32_t mode,uint32_t uid,uint32_t gid,uint32_t rdev,uint32_t inode) {
	fsnode *wd,*p;
	if (type!=TYPE_FILE && type!=TYPE_SOCKET && type!=TYPE_FIFO && type!=TYPE_BLOCKDEV && type!=TYPE_CHARDEV && type!=TYPE_DIRECTORY) {
		return ERROR_EINVAL;
	}
...
uint8_t slave_fs_unlink(uint32_t ts,uint32_t parent,uint32_t nleng,const uint8_t *name,uint32_t inode) {
	fsnode *wd;
	fsedge *e;
	wd = fsnodes_id_to_node(parent);
	if (!wd) {
...
uint8_t slave_fs_move(uint32_t ts,uint32_t parent_src,uint32_t nleng_src,const uint8_t *name_src,uint32_t parent_dst,uint32_t nleng_dst,const uint8_t *name_dst,uint32_t inode) {
	fsnode *swd;
        fsedge *se;
        fsnode *dwd;
        fsedge *de;
...
//              name_dst = fp->name;
        ...
        if (de) {
        	...
        	slave_fsnodes_unlink(ts,de);
        }
        slave_fsnodes_remove_edge(ts,se);
        fsnodes_link(ts,dwd,node,nleng_dst,name_dst);	
	version++;
	return STATUS_OK;
}

uint8_t slave_fs_link(uint32_t ts,uint32_t inode_src,uint32_t parent_dst,uint32_t nleng_dst,uint8_t *name_dst) {
	fsnode *sp;
        fsnode *dwd;
...
uint8_t slave_fs_snapshot(uint32_t ts,uint32_t inode_src,uint32_t parent_dst,uint16_t nleng_dst,uint8_t *name_dst,uint8_t canoverwrite) {
        //fsnode *rn;
        fsnode *sp;
        fsnode *dwd;
        uint8_t status;
	...
	slave_fsnodes_snapshot(ts,sp,dwd,nleng_dst,name_dst);
	version++;
	return STATUS_OK;
}	

uint8_t slave_fs_append(uint32_t ts,uint32_t inode,uint32_t inode_src) {
	uint8_t status;
        fsnode *p,*sp;
        if (inode==inode_src) {
                return ERROR_EINVAL;
        ...
        status = slave_fsnodes_appendchunks(ts,p,sp);
        if (status!=STATUS_OK) {
                return status;
        }
	version++;
...
uint8_t slave_fs_aquire(uint32_t inode,uint32_t sessionid) {
        fsnode *p;
        sessionidrec *cr;
        p = fsnodes_id_to_node(inode);
        if (!p) {
...
uint8_t slave_fs_release(uint32_t inode,uint32_t sessionid) {
        fsnode *p;
        sessionidrec *cr,**crp;
        p = fsnodes_id_to_node(inode);
        if (!p) {
...
uint8_t slave_fs_session(uint32_t sessionid) {
//	if (sessionid!=nextsessionid) {
//		return ERROR_MISMATCH;
//	}
    NOT_USED(sessionid);
	/**
	  * as master could core down when it have not store the nextsessionid in 
	  * metadata file, so we should increase it and therefore generate the 
	  * right sessionid 
	  * Dongyang Zhang
	  */
	version++;
...
uint8_t slave_fs_write(uint32_t ts,uint32_t inode,uint32_t indx,uint8_t opflag,uint64_t chunkid) {
	int status;
	uint32_t i;
	uint64_t ochunkid,nchunkid;
	fsnode *p;
	...
	ochunkid = p->data.fdata.chunktab[indx];
	status = slave_chunk_multi_modify(ts,&nchunkid,ochunkid,inode,indx,p->goal,opflag);
	if (status!=STATUS_OK) {
		return status;
	}
	if (nchunkid!=chunkid) {
...
uint8_t slave_fs_incversion(uint64_t chunkid) {
	version++;
	return chunk_increase_version(chunkid);
}

uint8_t slave_fs_repair(uint32_t ts,uint32_t inode,uint32_t indx,uint32_t nversion) {
	fsnode *p;
	uint8_t status;
	p = fsnodes_id_to_node(inode);
	if (!p) {
	...
	if (nversion==0) {
		status = slave_chunk_delete_file(p->data.fdata.chunktab[indx],inode,indx);
		p->data.fdata.chunktab[indx]=0;
	} else {
		status = slave_chunk_set_version(p->data.fdata.chunktab[indx],nversion);
	}
	version++;
	p->mtime = p->ctime = ts;
	return status;
...
uint8_t slave_fs_setgoal(uint32_t ts,uint32_t inode,uint32_t uid,uint8_t goal,uint8_t smode,uint32_t sinodes,uint32_t ncinodes,uint32_t nsinodes/*,uint32_t qeinodes*/) {
	uint32_t si,nci,nsi;
	fsnode *p;
...
uint8_t slave_fs_settrashtime(uint32_t ts,uint32_t inode,uint32_t uid,uint32_t trashtime,uint8_t smode,uint32_t sinodes,uint32_t ncinodes,uint32_t nsinodes) {
	uint32_t si,nci,nsi;
        fsnode *p;
...
uint8_t slave_fs_seteattr(uint32_t ts,uint32_t inode,uint32_t uid,uint8_t eattr,uint8_t smode,uint32_t sinodes,uint32_t ncinodes,uint32_t nsinodes) {
	uint32_t si,nci,nsi;
	fsnode *p;
	si = 0;
        nci = 0;
...
//slavemaster interface
uint8_t slave_fs_emptytrash(uint32_t ts,uint32_t freeinodes,uint32_t reservedinodes) {
        uint32_t fi,ri;
        fsedge *e;
        fsnode *p;
...
uint8_t slave_fs_emptyreserved(uint32_t ts,uint32_t freeinodes) {
	fsedge *e;
        fsnode *p;
        uint32_t fi;
	fi =0;
