...
typedef struct cblock_s {//块数据结构，一个chunk为64m，一个chunk包含1024个block，每个block为64k
...
typedef struct inodedata_s {//猜想是文件数据结构
	uint32_t inode;
	uint64_t maxfleng;
...
#endif
	for (i=0 ; i<WORKERS ; i++) {
		pthread_create(write_worker_th+i,&thattr,write_worker,(void*)(unsigned long)(i));//????????????????(void*)(unsigned long)(i)
	}
