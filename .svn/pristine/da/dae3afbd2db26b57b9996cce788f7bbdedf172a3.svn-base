...
//provide service till meta_ready
...
int matocuserv_load_sessions() {
	...
	while (!feof(fd)) {
		...
		if (r==1) {
			...
			sessionshead = asesdata;
			/*
			  * renew the nextsessionid, as the shadow master have its own 
			  * session with client, is the metadata.bk's nextsessionid is smaller
			  *  than the num of all session, two session may use the same 
			  * sessionid witch will lead client session lost 
			  * Dongyang Zhang 
			  */			
			if(asesdata->sessionid >= nextsessionid) {
				nextsessionid = asesdata->sessionid + 1;
			}
		}
		if (ferror(fd)) {
			MFSLOG(LOG_WARNING,"can't load sessions, fread error");
