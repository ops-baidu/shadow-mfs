...
void masterconn_changelog_rotate(serventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (length==1 && data[0]==0x55) {
		...
		MFSLOG(LOG_NOTICE,"MATOML_CHANGELOG_ROTATE wrong format");
	}
}
...
void masterconn_metachanges_log(serventry *eptr,const uint8_t *data,uint32_t length) {
	...
	while(log_count < log_limit) {
		...
		data = data + size;
		if (log_str[size - 1] != '\0') {
			MFSLOG(LOG_NOTICE,"MATOML_CHANGELOG - invalide string the last is %c",log_str[size - 1]);
		}
...
int masterconn_download_end(serventry *eptr,uint8_t filenum) {
	...
	eptr->downloading=0;
	buff = masterconn_createpacket(eptr,MLTOMA_DOWNLOAD_END,1);
	if (buff==NULL) {
		eptr->mode=KILL;
		return -1;
	}
...
int masterconn_download_next(serventry *eptr) {
	uint8_t *ptr;
	uint8_t filenum;
	uint64_t dltime;
	uint64_t starttime;
	...
	if (eptr->dloffset>=eptr->filesize) {	// end of file
		...
		/**
		  * as the metadata load or restore may cost much time, 
		  * we should reset the lastread incase the eptr is 
		  * timeout which would lead to metadata resend forever
		  * 
		  * Dongyang Zhang
		  */		
...
void masterconn_download_start(serventry *eptr,const uint8_t *data,uint32_t length) {
	if (length!=1 && length!=8) {
		MFSLOG(LOG_NOTICE,"MATOML_DOWNLOAD_START - wrong size (%"PRIu32"/1|8)",length);
		eptr->mode = KILL;
		return;
	}
	if (length==1) {
	...
	if (eptr->downloading==1) {
		...
		MFSLOG(LOG_NOTICE,"unexpected MATOML_DOWNLOAD_START packet");
		eptr->mode = KILL;
		return;
	}
	if (eptr->metafd<0) {
...
void masterconn_download_data(serventry *eptr,const uint8_t *data,uint32_t length) {
	...
	ssize_t ret;
	if (eptr->metafd<0) {
		MFSLOG(LOG_NOTICE,"MATOML_DOWNLOAD_DATA - file not opened");
		eptr->mode = KILL;
		return;
	}
	if (length<16) {
		MFSLOG(LOG_NOTICE,"MATOML_DOWNLOAD_DATA - wrong size (%"PRIu32"/16+data)",length);
		eptr->mode = KILL;
		return;
	}
	...
	crc = get32bit(&data);
	if (leng+16!=length) {
		MFSLOG(LOG_NOTICE,"MATOML_DOWNLOAD_DATA - wrong size (%"PRIu32"/16+%"PRIu32")",length,leng);
		eptr->mode = KILL;
		return;
	}
	if (offset!=eptr->dloffset) {
		MFSLOG(LOG_NOTICE,"MATOML_DOWNLOAD_DATA - unexpected file offset (%"PRIu64"/%"PRIu64")",offset,eptr->dloffset);
		eptr->mode = KILL;
		return;
	}
	if (offset+leng>eptr->filesize) {
		MFSLOG(LOG_NOTICE,"MATOML_DOWNLOAD_DATA - unexpected file size (%"PRIu64"/%"PRIu64")",offset+leng,eptr->filesize);
		eptr->mode = KILL;
		return;
	}
#ifdef HAVE_PWRITE
...
void masterconn_ack_changelog(serventry *eptr,int flag) {
	...
	ptr = masterconn_createpacket(eptr,MLTOMA_ACK_CHANGELOG,1);			
	if (ptr == NULL) {
		eptr->mode = KILL;
		return;
	}
...
void masterconn_sync_thread(serventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (flag == 1) {
		...
		MFSLOG(LOG_NOTICE,"MATOML_SYNC_THREAD:unrecognize flag");
	}
...
void masterconn_changelog_ready(serventry *eptr) {
	...
	ptr = masterconn_createpacket(eptr,MLTOMA_CHANGELOG_READY,0);
	if (ptr == NULL) {
		eptr->mode = KILL;
...
int masterconn_initlisten() {
        ...
        if (tcpstrlisten(lsock,ListenHost,ListenPort,100)<0) {
                MFSLOG(LOG_ERR,"smtoma: listen error: %m");
        ...
        fprintf(msgfd,"shadow master <-> master module: listen on %s:%s\n",ListenHost,ListenPort);
	return 0;
}
